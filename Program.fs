        ...
        async{
            ...
            let verb, response = 
                if requestedpage = "/" then 
                    ...
                    printfn "ICON"
                    PayloadVerb.GotFile,System.IO.File.ReadAllBytes("./HTML/resources/favicon.ico") 
                    ...
                    if System.IO.File.Exists("./HTML/"+requestedpage) then
                        //printf "Getting..."
                        let v = PayloadVerb.GotFile,System.IO.File.ReadAllBytes("./HTML/"+requestedpage) 
                        //printfn " Got"
                        v
                    else 
                    ...
                    |> string 
                    |> System.Text.ASCIIEncoding.ASCII.GetBytes)
            let r = {payloadsize = response.Length;payloadverb = verb; session = None; payload = response}
